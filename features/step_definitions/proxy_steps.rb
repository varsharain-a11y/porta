...
Then "I should be offered to download a(n) {string} file" do |mime_type|
...
When "the proxy has simple secret token" do
  ...
  proxy.update!(secret_token: 'simple')
end

And "all the apps have simple user keys" do
  Cinstance.where { user_key.not_eq nil }
           .update!(user_key: 'simple')
end
...
Then "The curl command uses {} with app_id/app_key credentials" do |authentication|
  matches = {
  ...
  assert matches.includes? authentication
  within '#api-test-curl' do
    page.should have_content(matches[authentication])
  end
end

Given "the service uses app_id/app_key as authentication method" do
