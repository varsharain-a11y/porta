...
module PathsHelper
  def path_to(*args)
    path_finder.path_to(*args)
  end

  protected

  def path_finder
    @path_finder ||= PathFinder.new(@provider)
  end
  ...
  class PathFinder
    ...
    def provider_api_docs_service_first
      @provider.api_docs_services.first!
    end

    def provider_default_service
